...
- Send suggested replies and actions alongside messages
...
formats output, and sets the exit code.
...
and response parsing. Uses `requests` for HTTP. All methods return structured
data or raise typed exceptions.
...
messages, rich cards, standalone rich cards, suggested replies, suggested
actions (dial, open URL), and carousels.
...
models.py (build message object)
 │
 ▼
client.py (authenticate, POST to RBM API)
 │
 ▼
...
- Network errors and timeouts are retried once, then reported.
- All errors are written to stderr. Normal output goes to stdout.

## Testing Strategy

- **Unit tests**: Mock HTTP responses to test client.py logic, test config
  loading with temp files, test model serialization.
...
- `google-auth-httplib2` >= 0.1 - Auth transport (unused at runtime but
  required by google-auth ecosystem)

Dev dependencies:
- `pytest` >= 7.0
- `responses` >= 0.22 (HTTP mocking)

## Future Considerations

- Support for carousel messages (multiple rich cards)
- File/media uploads for rich card images
